...
const mediaDistributionAliases = new Set([
  'mediadistribution',
  'media-distribution',
  'media_distribution',
  'press',
...
const getPostTokens = (post: SitePost) => {
  const content = getContent(post)
  const values = [
    asText(content.type),
    asText(content.task),
    asText(content.contentType),
    asText(content.kind),
    asText(content.route),
    asText(content.channel),
...
const isMediaDistributionPost = (post: SitePost) => {
  ...
  if (tokens.some((token) => token === 'comment')) return false
  return tokens.some((token) => mediaDistributionAliases.has(token))
}

async function fetchEditableMediaDistributionPosts({ page, limit, category }: { page: number; limit: number; category: string }) {
  ...
  const filtered = (feed?.posts || []).filter((post) => {
    if (!isMediaDistributionPost(post)) return false
    const postCategory = getCategory(post, '')
    return !normalizedCategory || normalizeCategory(postCategory) === normalizedCategory
  })
...
export async function EditableTaskArchiveRoute({
  ...
  const recovered = task === 'mediaDistribution' && !posts.length
    ? await fetchEditableMediaDistributionPosts({ page, limit: 24, category })
    : null
  const displayPosts = recovered?.posts || posts
  const displayPagination = recovered?.pagination || pagination
  return <TaskArchiveView task={task} posts={displayPosts} pagination={displayPagination} category={category} basePath={basePath || taskConfig?.route || `/${task}`} />
}
...
function EditorialArchive({
  ...
  const remaining = posts.slice(lead ? 1 : 0)

  return (
    <EditableSiteShell>
      <main className="media-shell min-h-screen">
        <section className="border-b border-white/10">
          <div className="mx-auto flex max-w-[1200px] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-16">
            <div>
              <p className="media-eyebrow">Media distribution archive</p>
              <h1 className="mt-3 text-5xl font-black leading-tight tracking-[-0.05em] text-white sm:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-white/62">
              Browse press releases, publisher-ready announcements, outreach campaigns, and distribution updates with clear campaign metadata.
            </p>
          </div>
        </section>

        <section className="border-b border-white/10 bg-black/20 text-white">
          <div className="mx-auto flex max-w-[1200px] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[var(--slot4-accent)]' : 'hover:text-[var(--slot4-accent)]'}>All campaigns</Link>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1200px] gap-5 px-4 py-8 sm:px-6 lg:grid-cols-[1.45fr_.75fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group media-card relative min-h-[34rem] overflow-hidden rounded-3xl">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#04100f]">{getCategory(lead, label)}</span>
                ...
                <div className="mt-6 flex flex-wrap gap-3 text-xs font-black uppercase tracking-[.12em]">
                  <span className="rounded-full bg-white/12 px-4 py-2">Reach 248K</span>
                  <span className="rounded-full bg-white/12 px-4 py-2">Status Published</span>
                  <span className="rounded-full bg-white/12 px-4 py-2">Engagement 14.8%</span>
                </div>
              </div>
            </Link>
            <div className="grid gap-5">
              <div className="media-card rounded-3xl p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">Campaigns gaining media momentum now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group media-card grid grid-cols-[7rem_1fr] overflow-hidden rounded-3xl">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-3 line-clamp-3 text-xl font-black leading-tight text-white">{post.title}</h3>
                    <p className="mt-3 text-xs font-bold text-white/48">Publisher: {SITE_CONFIG.name}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[1200px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-white/10 pb-5">
            <h2 className="text-4xl font-black tracking-[-0.04em] text-white sm:text-5xl">Campaign archive</h2>
            <form action={basePath} className="flex rounded-2xl border border-white/14 bg-white/[.04] p-1">
              <select name="category" defaultValue={category} className="h-11 min-w-44 bg-transparent px-3 text-xs font-black uppercase text-white outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="h-11 rounded-xl bg-[var(--slot4-accent)] px-5 text-xs font-black uppercase tracking-[0.14em] text-[#04100f]">Filter</button>
            </form>
          </div>

          {remaining.length ? (
            <div className="media-stagger grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group media-card overflow-hidden rounded-2xl">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-4 line-clamp-3 text-2xl font-black leading-tight text-white">{post.title}</h3>
                    <p className="mt-4 line-clamp-3 text-sm leading-6 text-white/56">{getSummary(post)}</p>
                    <div className="mt-5 grid grid-cols-2 gap-2 text-xs font-bold text-white/68">
                      <span className="rounded-xl bg-white/[.06] px-3 py-2">Brand {SITE_CONFIG.name}</span>
                      <span className="rounded-xl bg-white/[.06] px-3 py-2">Reach {84 + index * 17}K</span>
                      <span className="rounded-xl bg-white/[.06] px-3 py-2">Published</span>
                      <span className="rounded-xl bg-white/[.06] px-3 py-2">Engaged</span>
                    </div>
                    <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[var(--slot4-accent)]">View campaign <ArrowRight className="h-4 w-4" /></span>
                  </div>
                </Link>
              ))}
            </div>
          ) : !lead ? (
            <div className="media-card rounded-3xl border-dashed p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-black text-white">No campaigns found</h2>
              <p className="mt-2 text-sm text-white/60">Try another category or publish a new media distribution campaign.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-full border border-white/14 bg-white/[.04] px-5 py-3 text-xs font-black uppercase text-white">Previous</Link> : null}
            <span className="bg-[var(--slot4-accent)] px-5 py-3 text-xs font-black uppercase text-[#04100f]">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-full border border-white/14 bg-white/[.04] px-5 py-3 text-xs font-black uppercase text-white">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
